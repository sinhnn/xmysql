...
class Xsql {
  constructor(sqlConfig, pool) {
    ...
    this.autoAddUnknownColumns = true;
  }

  /* convert object to string */
  convert_to_basic_sql_data_type (obj) {
    var nd = {};
    for(var k in obj) {
      nd[k] = typeof obj[k] !== "object" ? obj[k] : JSON.stringify(obj[k]);
    }
    return nd;
  }
...
o
  ...
  prepareBulkInsert(tableName, objectArray, queryParamsObj) {
    if (tableName in this.metaDb.tables && objectArray) {
      ...
      var nObjectArray = [];
      var nObjectArray = objectArray.map(e=>this.convert_to_basic_sql_data_type(e));
      objectArray  = nObjectArray;
  ...
  async addUnknownColumns (inputColumns, tableName, dataTypes) {
    // update scheme
    let self = this;
    self.pool.query(
      dataHelp.getSchemaQuery(),
    ...
    // TODO: update list of column <09-02-21, sinhnn //
    let tableColumns = this.metaDb.tables[tableName]["columns"];
    // find unknown fields if any
    // var ncols = cols.filter(e => tableColumns.findIndex(c=>c["column_name"]===e) <= 0);
    var ncols = [];
    for (var j = 0; j < cols.length; ++j) {
      var i = tableColumns.findIndex(c=>c["column_name"] === cols[j]);
      if (i >= 0) continue;
      ncols.push(`ADD COLUMN \`${cols[j]}\` ${dataTypes[j]} DEFAULT NULL`);
    }
    if(ncols.length) {
      console.log("ALTER TABLE ?? " + ncols.join(','), [tableName]);
      await this.exec("ALTER TABLE ?? " + ncols.join(','), [tableName]);
    }
  }
