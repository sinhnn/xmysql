...
// TODO: FIXME <06-07-21, sinhnn> //
const r_sqlTypeRegex = /(?<type>[A-z]+)(\((?<length>[0-9]+)\))?/
const sqlType_formType = {
  "int": "number",
  "varchar": "text",
  "double": "number",
  "decimal": "number",
...
function mysqlType_to_InputType(type) {
  let m = type.match(r_sqlTypeRegex);
  return {"type": sqlType_formType[m.groups["type"]], "length": m.groups["length"]};
}
...
class xctrl {
  constructor(app, mysql) {
    this.app = app;
    this.mysql = mysql;
    // TODO: generate specific input form for each table <06-07-21, sinhnn> //
    this.templates = {
      "addOne" : {
        "__default__":  nunjucks.compile(fs.readFileSync('templates/form.html', 'utf8')),
        "rendered": {}
      }
    };
  ...
  async generateTemplates() {
    // generate all tempalte
  }

  async autoAddUnknownColumns(req) {
    if (this.mysql.autoAddUnknownColumns) {
      await this.mysql.addUnknownColumns(Object.keys(req.body),
        req.app.locals._tableName,
        Array.from(Object.keys(req.body)).map(e=>"mediumtext"),
      )
    }
  }
  ...
  async create(req, res) {
    ...
    await this.autoAddUnknownColumns(req);
    params.push(this.mysql.convert_to_basic_sql_data_type(req.body));
  ...
  async addOne(req, res) {
    // Convert table describe to html form
    let query = "describe ??";
    ...
    // TODO: ignore key has default value <06-07-21, sinhnn> //
    results = results.filter(e=>!e.Default);
    if( !this.templates["addOne"]["rendered"][tableName] ){
      this.templates["addOne"]["rendered"][tableName] = ".....";
      for (var i = 0, len = results.length; i < len; i++) {
        // TODO: move more to table attribute <06-07-21, sinhnn> //
        let prettyName = results[i]["Field"].replace(/_/g, " ").toUpperCase();
        let isRequired = results[i]["Key"] === "PRI" || results[i]["Null"] === "NO" && !results[i]["Default"];
        results[i]["prettyName"] = prettyName;
        results[i]["html"] = {"__attr__": []};
        results[i]["html"]["__attr__"].push({"name": "id", "value": results[i].Field});
        results[i]["html"]["__attr__"].push({"name": "name", "value": results[i].Field});
        results[i]["html"]["__attr__"].push({"name": "type", "value": mysqlType_to_InputType(results[i].Type)["type"]});
        results[i]["html"]["__attr__"].push({"name": "required", "value": isRequired ? "true" : null});
        results[i]["html"]["__attr__"].push({"name": "aria-required", "value": isRequired});
        results[i]["html"]["__attr__"].push({"name": "placeholder", "value": prettyName});
        results[i]["__attr__text" ] = Array.from(results[i]["html"]["__attr__"]).filter(e=>e.value!=null).map(e=>`${e.name}=${e.value}`).join(" ");
      }
      this.templates["addOne"]["rendered"][tableName] = this.templates["addOne"]["__default__"].render({
        "items": results,
        "action": req.url.replace(/\/addOne.*$/, ''),
        "table": req.app.locals._tableName
      })
    }
    res.status(200).send(this.templates["addOne"]["rendered"][tableName]);
  }
  ...
  async patch(req, res) {
    ...
    params = params.concat(Object.values(this.mysql.convert_to_basic_sql_data_type(req.body)));
  ...
  async bulkInsert(req, res) {
    ...
    var n = JSON.parse(JSON.stringify(req.body[0]));
    if (this.mysql.autoAddUnknownColumns) {
        await this.mysql.addUnknownColumns(Object.keys(n),
          req.app.locals._tableName,
          Array.from(Object.keys(n)).map(e=>"mediumtext"),
        )
    }
