...
function mysqlType_to_InputType(type) {
  if (["int", "float", "double", "timestamp", "integer"].indexOf(type) >= 0) {
    return "number";
  }
  return "text"
}
...
class xctrl {
  constructor(app, mysql) {
    ...
    this.templates = {
      "addOne" : {
        "__default__":  nunjucks.compile(fs.readFileSync('templates/form.jinja2', 'utf8')),
        "rendered": {}
      }
    };
  ...
  async addOne(req, res) {
    ...
    if( !this.templates["addOne"]["rendered"][tableName] ){
      for (var i = 0, len = results.length; i < len; i++) {
        ...
        results[i]["html"]["__attr__"].push({"name": "type", "value": mysqlType_to_InputType(results[i].Type)});
        results[i]["html"]["__attr__"].push({"name": "required", "value": results[i]["Key"] === "PRI" ? "true" : null});
        results[i]["html"]["__attr__"].push({"name": "aria-required", "value": results[i]["Key"] === "PRI"});
        results[i]["html"]["__attr__"].push({"name": "placeholder", "value": prettyName});
        // results[i]["html"]["__attr__"].push({"name": "maxlength", "value": results[i]["Key"] === "PRI"});
        results[i]["__attr__text"] = Array.from(results[i]["html"]).filter(e=>e.value != null).map(e =>`${e.name}={e.value}`);
        // results[i]["required"] = results[i]["Key"] === "PRI" ? "require" : "";
        // results[i]["type"] = mysqlType_to_InputType(results[i].Type);
      }
